	...
	public void NotifyCovid(ArrayList<SuspectedCovid> suspectC, String nric) throws RemoteException {
		...
		for (SuspectedCovid sc : suspectC) {
			if(sc.getNric().contains(nric)) {
				System.out.println("Possible exposure @ : " + sc.getLocation() + ", on " + sc.getDateSuspected() + "\n");
			}
		}
		System.out.println("\n--------------------------");
	 ...
	 /**
	  * Function takes in user's input for indvidual check in and invoke check-in function in the RMI Server 
	  * @param sc- Scanner to scan user input for multiple check in
	  * @param service- used to invoke RMI Server function
	  * @param client - pass client through invoked function for callback
	  * @param nric - nric of the current user of the client
	  * @param name - name of the current user of the client
	 ...
	 /**
	  * Function takes in user's input for indvidual check out and invoke check-out function in the RMI Server 
	  * @param sc- Scanner to scan user input for multiple check in
	  * @param service- used to invoke RMI Server function
	  * @param client - pass client through invoked function for callback
	  * @param nric - nric of the current user of the client
	 ...
	 /**
	  * Function takes in user's input for multiple check out and invoke multiple-checkIn function in the RMI Server 
	  * @param sc- Scanner to scan user input for multiple check in
	  * @param service- used to invoke RMI Server function
	  * @param client - pass client through invoked function for callback
	  * @param nric - nric of the current user of the client
